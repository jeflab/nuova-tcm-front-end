 ...
 */
export interface TypedFormData<T> {
  /**
...
export function getTypedFormData<T>(
  form?: HTMLFormElement | null,
): TypedFormData<T> {
  return new FormData(form || undefined) as unknown as TypedFormData<T>;
}

export function getTypedFormDataFromObject<T>(object: T): TypedFormData<T> {
  const formData = new FormData() as unknown as TypedFormData<T>;
